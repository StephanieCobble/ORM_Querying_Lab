...
def problem_three(request):
  ...
  # for instructor_id in instructor:
  instructor_course = Course.objects.filter(instructor=instructor_two)
  print(f'Instructor Name: {instructor_two.first_name} {instructor_two.last_name}')
  
  for course_name in instructor_course:
    print(f'Courses: {course_name.name}')
...
def problem_five(request):
  student = Student.objects.create(first_name='Stephanie', last_name='Cobble', year=2022, gpa=4.0)
...
def problem_six(request):
    ...
    student_id = 13
    Student.objects.filter(pk=student_id).update(gpa=4.1)
    student = Student.objects.get(pk=13)
    print(f'ID: {student.id} \nName: {student.first_name} {student.last_name} \nGPA: {student.gpa}')
